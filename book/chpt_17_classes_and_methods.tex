...
修改 \li{Time} 类的属性，使用一个整数代表自午夜零点开始的秒数。
然后修改类的方法（和 \li{int_to_time} 函数 ），使其适用于新的实现。
你不用修改 \li{main} 函数中的测试代码。  完成之后，程序的输出应该和之前保持一致。
...
\item 一个 \li{__init__} 方法，初始化一个叫 \li{pounch_contents} 的属性为空列表。

\item 一个叫 \li{put_in_pounch} 的方法，将一个任意类型的对象加入 \li{pounch_contents}。

\item 一个 \li{__str__} 方法，返回 \li{Kangaroo} 对象的字符串表示和 \li{pounch} 中的内容。
...
创建两个 \li{Kangaroo} 对象，将它们命名为 \li{kanga} 和 \li{roo} ，然后将 \li{roo} 加入 \li{kanga} 的 \li{pounch} 列表，以此测试你写的代码。
