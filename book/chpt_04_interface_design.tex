...
\item 画一个执行 {\em \li{circle(bob, radius)}} 时的堆栈图 (stack diagram) ，说明程序的各个状态。你可以手动进行计算，也可以在代码中加入打印语句。

\item Section~\ref{refactoring} 中给出的 \li{arc} 函数版本并不太精确，因为圆形的线性近似（linear approximation）永远处在真正的圆形之外。因此，\li{Turtle} 总是和正确的终点相差几个像素。我的答案中展示了降低这个错误影响的一种方法。阅读其中的代码，看看你是否能够理解。如果你画一个堆栈图的话，你可能会更容易明白背后的原理。
...
你应该为每个字母写一个函数，起名为 \li{draw_a}，\li{draw_b} 等等，
然后将你的函数放在一个名为 \li{letters.py} 的文件里。
